...
//            }
    ...
    public void setConfig(RecordConfig config) {
        recColor = config.timelineColor;
        cutColor = config.timelineColor;
    }

    void create() {
        handler = new Handler();

        pitchDlimiter = Utils.dp2px(getContext(), PITCH_DELIMITER);
        pitchWidth = Utils.dp2px(getContext(), PITCH_WIDTH);
...
//            }
//        };
//        fft.setPadding(0, Utils.dp2px(2), 0, 0);
//        addView(fft);

        current = new PitchCurrentView(getContext());
        current.setPadding(0, Utils.dp2px(getContext(), 2), 0, 0);
        addView(current);
